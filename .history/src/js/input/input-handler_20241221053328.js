class InputHandler {
  ...
  _handleInputError(error, context) {
    console.error(`Input error (${context}):`, error);
    this.errorCount++;

    if (this.errorCount >= this.MAX_ERRORS) {
      NotificationManager.showError(
        "Too many input errors. Stopping control session."
      );
      this.stopControl();
    } else {
      NotificationManager.showWarning(`Input error: ${error.message}`);
    }
  }
  ...
  _removeEventListeners() {
   ...
   _handleMouseEvent(event) {
        ...
        try {
            ...
            this._handleInputError(error, 'mouse event');
        }
    }
