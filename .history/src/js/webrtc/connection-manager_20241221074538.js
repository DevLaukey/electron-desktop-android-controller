const notify = {
    success: (message) => {
        if (window.electronAPI) {
            window.electronAPI.showNotification({ title: 'Success', body: message, type: 'success' });
        } else {
            console.log('✅', message);
        }
    },
    error: (message) => {
        if (window.electronAPI) {
            window.electronAPI.showNotification({ title: 'Error', body: message, type: 'error' });
        } else {
    ...
    info: (message) => {
        if (window.electronAPI) {
            window.electronAPI.showNotification({ title: 'Info', body: message, type: 'info' });
        } else {
...
export class ConnectionManager {
        ...
        try {
            ...
            setTimeout(() => {
                if (!this.socket.connected) {
                    notify.error(`Unable to connect to server at ${serverUrl}. Please check if the server is running.`);
                }
            }, 5000);

        } catch (error) {
            // Handle invalid URLs or connection setup failures
            notify.error(`Invalid server URL or connection setup failed: ${error.message}`);
            throw error;
        }
    }
    ...
    _setupSocketErrorHandling() {
        this.socket.on("connect_error", (error) => {
            notify.error(`Server connection failed: ${error.message}. Please check if the server is running.`);
            console.error("Socket connection error:", error);
        });

        this.socket.on("connect_timeout", () => {
            notify.error("Server connection timed out. Please check your internet connection and server status.");
        });
        ...
        this.socket.on("reconnect_failed", () => {
            notify.error("Failed to reconnect to server. Please check if the server is running.");
        });
        ...
        this.socket.on("disconnect", (reason) => {
            ...
            if (reason === 'io server disconnect') {
                ...
                notify.info('The server has disconnected. You may need to reconnect manually.');
            } else {
                // Connection lost
    ...
    async initializeConnection() {
        try {
            if (!this.socket.connected) {
                throw new Error('Not connected to server. Please check your connection and try again.');
            }
